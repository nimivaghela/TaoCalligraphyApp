package com.app.taocalligraphy.ui.experience.tablet
...
@AndroidEntryPoint
class ExperienceMoreTabletActivity : BaseActivity<ActivityExperienceMoreBinding>(),
    ...
    companion object {
        ...
        fun startActivity(activity: AppCompatActivity) {
            val intent = Intent(activity, ExperienceMoreTabletActivity::class.java)
    ...
    override fun initView() {
        if (!viewModel.isFromConfigChanges) {
            if (isNetwork()) {
    ...
    override fun handleListener() {
        ...
        mBinding.btnSignUpExperienceMore.setOnClickListener {
            ...
            WelcomeLoginActivity.startActivity(this@ExperienceMoreTabletActivity)
        ...
        mBinding.ivMediaFullScreen.setOnClickListener {
            ...
            isFromFullScreen = true
            viewModel.userExperienceDetail?.initialContent?.let {
                FullScreenPlayerTabletActivity.startActivity(
                    this,
...
//        )
    ...
    override fun onResume() {
        if (isPause) {
            ...
            if (isAlreadyLoaded) {
                if (!isFromFullScreen && !isMoveToPlayer && !viewModel.isFromConfigChanges) {
                    TaoCalligraphy.instance.isPlayerPlaying = true
                    setVideoDataInPlayer()
                }
            }
        }
        if (isAlreadyLoaded || viewModel.isFromConfigChanges)
            handlerMainTimer.postDelayed(runnableMainTimer, 0)
        isPause = false
        isMoveToPlayer = false
        super.onResume()
    }
...
//        TaoCalligraphy.instance.simpleExoPlayer?.playWhenReady = false
    ...
    override fun onDestroy() {
        stopNotificationService()
        LocalBroadcastManager.getInstance(this).unregisterReceiver(playerNotificationReceiver)
        viewModel.isFromConfigChanges = isChangingConfigurations
