...
class FullScreenVideoPlayerActivity : BaseActivity<ActivityFullScreenVideoPlayerBinding>() {
    ...
    companion object {
        ...
        var instance: FullScreenVideoPlayerActivity? = null
        ...
        fun startActivity(
            ...
            val intent = Intent(activity, FullScreenVideoPlayerActivity::class.java)
...
//        requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_USER_LANDSCAPE
    ...
    private fun setData() {
        StartPlayerActivity.instance.mViewModel.meditationContent?.let {
//            showProgressIndicator(mBinding.llProgress, false)
    ...
    private fun setVideoDataInPlayer() {
        TaoCalligraphy.instance.simpleExoPlayer?.run {
            addListener(object : Player.Listener {
                ...
                override fun onPlaybackStateChanged(playbackState: Int) {
                    if (playbackState == Player.STATE_ENDED) {
                        handlerControllerHide.removeCallbacks(runnableShowHideControl)
                        mBinding.ivMediaPlayPlause.setImageResource(R.drawable.vd_pause)
                        this@FullScreenVideoPlayerActivity.isPlaying = false
                    }
                }

                override fun onPlayWhenReadyChanged(playWhenReady: Boolean, reason: Int) {
                    this@FullScreenVideoPlayerActivity.isPlaying = playWhenReady
                    if (playWhenReady) {
                        StartPlayerActivity.instance.run {
                            //contentHandlerTimer.postDelayed(contentPlayTimeRunner, 1000)
                        }
                        handlerControllerHide.postDelayed(runnableShowHideControl, 2000)
            ...
            playbackSpeed = StartPlayerActivity.instance.playerPlaybackSpeed
    ...
    private fun setSubtitleView() {
        if (StartPlayerActivity.instance.mViewModel.meditationContent?.subtitleWithLanguages.isNullOrEmpty()) {
            mBinding.ivSubtitle.gone()
        } else {
            mBinding.ivSubtitle.visible()
        }
        if (StartPlayerActivity.instance.subtitleWithLanguage == null) {
    ...
    override fun handleListener() {
        ...
        mBinding.ivMediaPlayPlause.setOnClickListener {
            if (TaoCalligraphy.instance.simpleExoPlayer?.isPlaying == true) {
                ...
                if (!StartPlayerActivity.instance.mViewModel.isZenModeDontShow && userHolder.isZenModeForMeditation)
                    showZenModeDialog()
            }
            TaoCalligraphy.instance.simpleExoPlayer?.run {
        ...
        mBinding.ivSubtitle.setOnClickListener {
            StartPlayerActivity.instance.mViewModel.meditationContent?.let {
                if (StartPlayerActivity.instance.subtitleWithLanguage == null) {
                    ...
                    StartPlayerActivity.instance.subtitleWithLanguage = null
                    setSubtitleView()
                }
            }
        }
...
//            requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_SENSOR
        ...
        mBinding.zenModeView.btnDontShowAgain.setOnClickListener {
            StartPlayerActivity.instance.mViewModel.isZenModeDontShow = true
            StartPlayerActivity.instance.timer.cancel()
            zenModeTimer.removeCallbacks(zenModeTimerRunner)
    ...
    fun setSubtitleAndShow(subtitle: MeditationContentResponse.SubtitleWithLanguage?) {
        StartPlayerActivity.instance.subtitleWithLanguage = subtitle
        setSubtitleView()
        TaoCalligraphy.instance.simpleExoPlayer?.run {
        ...
        override fun run() {
            if ((!isFinishing)) {
                ...
                handlerMainTimer.postDelayed(
                    this,
                    (1000 / StartPlayerActivity.instance.playerPlaybackSpeed).toLong()
                )
            }
        }
    }
    ...
    private fun getTrackWiseMediaSource(): MergingMediaSource {
        ...
        val contentFile: String? = if (!StartPlayerActivity.instance.mViewModel.meditationContent?.contentFileForDownload.isNullOrEmpty()) {
            StartPlayerActivity.instance.mViewModel.meditationContent?.contentFileForDownload
        } else {
            StartPlayerActivity.instance.mViewModel.meditationContent?.contentFile
        }
        ...
        if (isInitialUser == true) {
            val mediaSource =
                HlsMediaSource.Factory(DefaultHttpDataSource.Factory().setUserAgent(userAgent))
                    .createMediaSource(
                        MediaItem.fromUri(Uri.parse(StartPlayerActivity.instance.mViewModel.meditationContent?.contentFileForHls))
                    )
        ...
        if (StartPlayerActivity.instance.subtitleWithLanguage == null) {
            ...
            val subTitle = StartPlayerActivity.instance.subtitleWithLanguage?.subTitleFile
...
//                .createMediaSource(uriSubtitle, textFormat, C.TIME_UNSET)
    ...
    private fun showPlaybackDialog() {
        ...
        settingPopupWindow?.contentView?.setSelectedPlaybackSpeed(
            this,
            StartPlayerActivity.instance.playerPlaybackSpeed
        )
    }
    ...
    private fun initializeSettingPopupWindow() {
        ...
        tv5Speed.setOnClickListener {
            StartPlayerActivity.instance.playerPlaybackSpeed = 0.5f
            settingPopupWindow?.contentView?.setSelectedPlaybackSpeed(
                this,
                StartPlayerActivity.instance.playerPlaybackSpeed
            )
        ...
        tv75Speed.setOnClickListener {
            StartPlayerActivity.instance.playerPlaybackSpeed = 0.75f
            settingPopupWindow?.contentView?.setSelectedPlaybackSpeed(
                this,
                StartPlayerActivity.instance.playerPlaybackSpeed
            )
        ...
        tvNormalSpeed.setOnClickListener {
            StartPlayerActivity.instance.playerPlaybackSpeed = 1f
            settingPopupWindow?.contentView?.setSelectedPlaybackSpeed(
                this,
                StartPlayerActivity.instance.playerPlaybackSpeed
            )
        ...
        tv15Speed.setOnClickListener {
            StartPlayerActivity.instance.playerPlaybackSpeed = 1.5f
            settingPopupWindow?.contentView?.setSelectedPlaybackSpeed(
                this,
                StartPlayerActivity.instance.playerPlaybackSpeed
            )
        ...
        tv2Speed.setOnClickListener {
            StartPlayerActivity.instance.playerPlaybackSpeed = 2f
            settingPopupWindow?.contentView?.setSelectedPlaybackSpeed(
                this,
                StartPlayerActivity.instance.playerPlaybackSpeed
            )
        ...
        tvSlower.setOnClickListener {
            when (StartPlayerActivity.instance.playerPlaybackSpeed) {
                0.75f -> {
                    StartPlayerActivity.instance.playerPlaybackSpeed = 0.5f
                }
                1f -> {
...
//        } else
    ...
    override fun onPause() {
        ...
        TaoCalligraphy.instance.simpleExoPlayer?.run {
            ...
            if (!isBackPressed) {
//                playWhenReady = false
                this@FullScreenVideoPlayerActivity.isPlaying = play
            }
    ...
    override fun onDestroy() {
        ...
        if (isNetwork() && isInitialUser == false && !isBackPressed)
            StartPlayerActivity.instance.callPlayTimeAPI()
        handlerMainTimer.removeCallbacks(runnableMainTimer)
        handlerControllerHide.removeCallbacks(runnableShowHideControl)
        instance = null
    }
...
//            mBinding.ivMediaFullScreen.performClick()
            onBackPressed()
