...
@AndroidEntryPoint
class StartPlayerActivity : BaseActivity<ActivityStartPlayerBinding>(),
    ...
    companion object {
        lateinit var instance: StartPlayerActivity
        ...
        fun startActivity(
            ...
            val intent = Intent(activity, StartPlayerActivity::class.java)
...
//            intent.addFlags(FLAG_ACTIVITY_NEW_TASK)
    ...
    val bottomSheetBehavior: BottomSheetBehavior<*> by lazy {
        return@lazy BottomSheetBehavior.from<View>(mBinding.llUpComingPlaylist)
    }
    ...
    private var isMoveToFullScreen = false
    ...
    private val playlistContentFilterList =
        mutableListOf<PlaylistContentFilterApiResponse.ContentList>()

    private var fullScreenMediaPlayerResult = registerForActivityResult(
        ActivityResultContracts.StartActivityForResult()
    ) {
        isMoveToFullScreen = false
...
//        requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED
        ...
        requestedOrientation = if (FullScreenVideoPlayerActivity.isOrientationButtonClicked)
    ...
    fun logIntentBundle() {
        ...
        if (bundle != null) {
            for (key in bundle.keySet()) {
                Log.e(
                    TAG,
                    "Intent :- " + key + " : " + if (bundle[key] != null) bundle[key] else "NULL"
                )
            }
        }
    }
    ...
    override fun initView() {
        ...
        if (!isNetwork() && getMeditationData() == null) {
            ...
            if (getMeditationData() != null)
                mViewModel.meditationContent = getMeditationData()
        }
        ...
        if (videoType == Constants.playlistVideo) {
            ...
            val layoutParams = mBinding.rrPlayerController.layoutParams as LinearLayout.LayoutParams
            layoutParams.setMargins(0, 0, 0, resources.getDimension(R.dimen._100sdp).toInt())
        ...
        } else if (programId != null && isNetwork()) {
            callPlaylistListAPI()
        } else {
            setData(true)
        }
        ...
        LocalBroadcastManager.getInstance(this@StartPlayerActivity).registerReceiver(
    ...
    val playerNotificationReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            when (intent?.action) {
                ...
                ACTION_CUSTOM_STOP -> {
//                    showToast(context ?: this@StartPlayerActivity, "ACTION_STOP CALLED")
                }
                ACTION_CUSTOM_REWIND -> {
//                    showToast(context ?: this@StartPlayerActivity, "ACTION_REWIND CALLED")
                    mBinding.ffBackForwardMedia.performClick()
                }
                ACTION_CUSTOM_FAST_FORWARD -> {
//                    showToast(context ?: this@StartPlayerActivity, "ACTION_FAST_FORWARD CALLED")
                    mBinding.ffMoveForwardMedia.performClick()
                }
                ACTION_CUSTOM_NEXT -> {
//                    val isActionEnabled = intent.getBooleanExtra("isActionEnabled", true)
//                    if (isActionEnabled) {
//                    showToast(context ?: this@StartPlayerActivity, "ACTION_NEXT CALLED")
                    if (videoType != Constants.meditationVideo && !programId.isNullOrEmpty()) {
                        if (mPlaylistViewModel.selectedPlaylistPosition < (playlistContentFilterList.size - 1)) {
...
//                    } else {
//                        showToast(context ?: this@StartPlayerActivity, "ACTION IS DISABLED")
//                    }
                }
                ACTION_CUSTOM_PREVIOUS -> {
//                    val isActionEnabled = intent.getBooleanExtra("isActionEnabled", true)
//                    if (isActionEnabled) {
//                    showToast(context ?: this@StartPlayerActivity, "ACTION_PREVIOUS CALLED")
                    if (videoType != Constants.meditationVideo && !programId.isNullOrEmpty()) {
...
//                    }
    ...
    private fun setData(fromInit: Boolean = false) {
        ...
        mViewModel.meditationContent?.let {
            ...
            handlerMainTimer.postDelayed(runnableMainTimer, (1000 / playerPlaybackSpeed).toLong())

            if (it.type != Constants.TEXT) {
                FullScreenVideoPlayerActivity.instance?.initView()
            } else {
                FullScreenVideoPlayerActivity.instance?.finish()
            }
    ...
    private fun setVideoDataInPlayer() {
        ...
        if (mIsBound && playerNotificationService != null) {
            ...
            if (videoType != Constants.meditationVideo && !programId.isNullOrEmpty()) {
                ...
                enableNextAction =
                    mPlaylistViewModel.selectedPlaylistPosition < (playlistContentFilterList.size - 1)
            }
            mViewModel.meditationContent?.let {
            ...
            response?.let { requestState ->
                ...
                requestState.apiResponse?.data?.let {
                    ...
                    if (list.isNotEmpty()) {
                        ...
                        mPlaylistViewModel.selectedPlaylistPosition =
                            list.indexOf(firstContent)
                        playlistContentFilterList.clear()
                        playlistContentFilterList.addAll(list)
                        playlistViewItemAdapter.updateList(playlistContentFilterList)
                        setNextContentDataAndCallAPI()
                    }
                }
    ...
    override fun handleListener() {
        ...
        mBinding.ivMediaFullScreen.setOnClickListener {
            isMoveToFullScreen = true
            isFromFullScreen = true
            FullScreenVideoPlayerActivity.isOrientationButtonClicked = false
            FullScreenVideoPlayerActivity.startActivity(
                this,
                fullScreenMediaPlayerResult,
                isInitialUser, isFromFullScreen
            )
        }
...
//        )
        ...
        TaoCalligraphy.instance.simpleExoPlayer?.addListener(object : Player.Listener {
            override fun onPlaybackStateChanged(playbackState: Int) {
                if (playbackState == Player.STATE_BUFFERING) {
                    contentHandlerTimer.removeCallbacks(contentPlayTimeRunner)
                }

                if (playbackState == Player.STATE_READY) {
                    contentHandlerTimer.removeCallbacks(contentPlayTimeRunner)
                    contentHandlerTimer.postDelayed(contentPlayTimeRunner, 0)
                    if (TaoCalligraphy.instance.simpleExoPlayer?.playWhenReady == true) {
                        isPlaying = true

                        Constants.NOTIFICATION_ON_OFF = false
                        if (isPreparing && !zenModeDisplayed && !mViewModel.isZenModeDontShow && userHolder.isZenModeForMeditation)
                ...
                if (playbackState == Player.STATE_ENDED) {
                    if (TaoCalligraphy.instance.simpleExoPlayer?.repeatMode ==
                        ExoPlayer.REPEAT_MODE_OFF
                    ) {
                        isPlaying = false
                        ...
                        if (videoType == Constants.meditationVideo) {
                            ...
                            if (mPlaylistViewModel.selectedPlaylistPosition < (playlistContentFilterList.size - 1)) {
                                mPlaylistViewModel.selectedPlaylistPosition =
                                    mPlaylistViewModel.selectedPlaylistPosition + 1
                                setNextContentDataAndCallAPI()
                            } else if (mPlaylistViewModel.selectedPlaylistPosition == (playlistContentFilterList.size - 1)) {
                                FullScreenVideoPlayerActivity.instance?.finish()
                                onBackPressed()
                            }
            ...
            override fun onPlayerError(error: PlaybackException) {
                ...
                this@StartPlayerActivity.showCustomDialog(getString(R.string.content_not_load_msg),
    ...
    fun setNextContentDataAndCallAPI() {
        settingPopupWindow?.dismiss()
        callPlayTimeAPI()
        val content = playlistContentFilterList[mPlaylistViewModel.selectedPlaylistPosition]
        ...
        if (mPlaylistViewModel.selectedPlaylistPosition == (playlistContentFilterList.size - 1)) {
            ...
            val nextContent =
                playlistContentFilterList.getOrNull(mPlaylistViewModel.selectedPlaylistPosition + 1)
            if (nextContent != null) {
    ...
    fun openMeditationFeedbackScreen() {
        ...
        if (isFromMeditate) {
            ...
            (mViewModel.meditationContent?.isShowRating == true &&
                    (mViewModel.meditationContent?.isAssessmentDone == false ||
                            mViewModel.meditationContent?.isPostAssessmentCompleted == false))
        ) {
...
//                playWhenReady = false
                this@StartPlayerActivity.isPlaying = play
...
//        Log.e(TAG, "startNotificationService")
        notificationServiceIntent = Intent(this, ExoNotificationService::class.java)
            .putExtra(Constants.MeditationContent, mViewModel.meditationContent)
...
//        }
    ...
    override fun onDestroy() {
        ...
        LocalBroadcastManager.getInstance(this).unregisterReceiver(playerNotificationReceiver)
        LocalBroadcastManager.getInstance(this@StartPlayerActivity)
    ...
    fun callPlayTimeAPI() {
        val currentTimeValue = String.format(
            "%02d:%02d:%02d",
            TimeUnit.MILLISECONDS.toHours(totalContentTime),
            TimeUnit.MILLISECONDS.toMinutes(totalContentTime) - TimeUnit.HOURS.toMinutes(
                TimeUnit.MILLISECONDS.toHours(totalContentTime)
            ),
            TimeUnit.MILLISECONDS.toSeconds(totalContentTime) - TimeUnit.MINUTES.toSeconds(
                TimeUnit.MILLISECONDS.toMinutes(totalContentTime)
            )
        )
        if (isFromProgram == true) {
        ...
        totalContentTime = 1000L
    }

    override fun onMediaChanged(position: Int) {
        val seekDuration =
            meditationVideoPlaylistAdapter.chapterList[position]?.startTimeStamp
        seekDuration?.let {
    ...
    fun setOrientationListener() {
        orientationEventListener = object : OrientationEventListener(this@StartPlayerActivity) {
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        ...
        mViewModel.meditationContent?.let {
            if (it.type != Constants.TEXT &&
                ...
                if (it.type != Constants.TEXT) {
                    if (!FullScreenVideoPlayerActivity.isOrientationButtonClicked) {
                        isMoveToFullScreen = true
                        FullScreenVideoPlayerActivity.startActivity(
                            this,
                            fullScreenMediaPlayerResult,
