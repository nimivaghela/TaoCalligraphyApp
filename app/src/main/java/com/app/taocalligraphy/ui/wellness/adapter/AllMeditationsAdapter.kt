...
import com.app.taocalligraphy.databinding.ItemAllMeditationsBinding
...
class AllMeditationsAdapter(var onMeditationItemClickListener: OnMeditationItemClickListener) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == ITEM_LOAD) {
            ...
            val itemBindingUtil =
                ItemAllMeditationsBinding.inflate(
                    LayoutInflater.from(parent.context),
                    parent,
                    false
                )
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ProgressViewHolder) {
            holder.itemView.updateLayoutParams {
                if(isTablet(TaoCalligraphy.instance)) {
                    height = holder.itemView.context.resources.getDimension(R.dimen._30sdp).toInt()
                }else{
...
//                    can access
    ...
    fun setMeditationData(list: ArrayList<ProgramDataModel?>) {
        this.list = list
        notifyDataSetChanged()
    }

    class DataHolder(var binding: ItemAllMeditationsBinding) :
